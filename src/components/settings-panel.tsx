  ...
  const handleTest = async (id: keyof ApiKeys) => {
    ...
    if (id === 'googleai') {
      ...
      setTimeout(() => {
        ...
        setApiKeys(prev => ({
          ...prev,
          [id]: { ...prev[id], status: isSuccess ? 'connected' : 'disconnected' }
        }));
         toast({
            title: isSuccess ? `${id.charAt(0).toUpperCase() + id.slice(1)} Connected` : "Connection Failed",
            description: isSuccess ? `Successfully connected to ${id}.` : `Could not connect to ${id}. Please check your key.`,
              ...
              <CardHeader className="flex flex-row items-center justify-between pb-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <CardDescription>Configure your {service.name} API key</CardDescription>
                  </div>
                </div>
