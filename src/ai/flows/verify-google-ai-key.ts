...
const VerifyGoogleAIKeyOutputSchema = z.object({
  ...
  message: z
    .string()
    .optional()
    .describe('An error message if the key is invalid.'),
});
    ...
    try {
      ...
      if (error.message) {
        if (error.message.includes('API key not valid')) {
          ...
          message =
            'The provided API key does not have access to the required models.';
        } else {
          message =
            'Failed to verify the key. It might be invalid, expired, or lack permissions.';
        }
      }
